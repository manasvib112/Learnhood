...
const Login = (props) => {
  ...
  const handleSubmit=async(event)=>{
    ...
    if(valid){
      await _loginModal.login(login).then((data)=>{
        // console.log(data);
        history.push("/home")
      }
      ).catch((err)=>{
  ...
  return (
    ...
    <img classname="Ellipse" alt="learnhood" src={Ellipse}></img> 
      ...
      {
        error && error.email ? <span className="error-message">{error.email[0]}</span>:null
      }
      <input name="password" type="password" placeholder="Password" onChange={handleChange} value={login.password} ></input>
      {
        error && error.password ? <span className="error-message">{error.password[0]}</span>:null
      }
